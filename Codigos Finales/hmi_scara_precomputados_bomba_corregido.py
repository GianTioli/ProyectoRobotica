...
def grupo_nombre(hoyos):
    return "grupo_" + "_".join(str(i) for i in hoyos) + ".csv"
...
def cargar_trayectoria(path):
    ...
    with open(path, newline="") as file:
        ...
        for row in reader:
            ...
            try:
                trayectoria.append((float(row[0]), float(row[1]), float(row[2]), float(row[3])))
            except ValueError:
                continue
...
class ScaraHMI:
    ...
    def selected_files(self):
        ...
        if len(selected) == 1:
            return [csv_path(f"pozo_{selected[0]}.csv")]

        if len(selected) == 6:
            return [csv_path("todos.csv")]

        return [csv_path(grupo_nombre(selected))]
    ...
    def send_reset(self, ser):
        ser.write(b"RESET\n")
        self.log("RESET enviado al Arduino")
        self.wait_ok(ser)

    def send_trajectory(self, ser, trayectoria):
        total = len(trayectoria)

        for i, (q1, q2, q3, q4) in enumerate(trayectoria, start=1):
            msg = f"{q1:.3f},{q2:.3f},{q3:.3f},{q4:.3f}\n"
            ser.write(msg.encode())
            self.log(f"Punto {i}/{total}: {msg.strip()}")
            self.wait_ok(ser)

    def wait_ok(self, ser):
        while True:
