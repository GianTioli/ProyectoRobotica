%% SCARA minimo: puntos -> trayectoria -> CSV
clear; clc; close all;
...
[P, names, dwell] = construirPuntos(robot, actuator, tubo, pozos_orden, task);
validarPuntos(P, cfg);
...
writematrix(tabla_csv, 'trayectoria_scara.csv');

fprintf('CSV generado: trayectoria_scara.csv\n');
...
function [P, names, dwell] = construirPuntos(robot, actuator, tubo, pozos, task)
    ...
    for i = 1:size(pozos,1)
        [P, names, dwell] = agregarPunto(P, names, dwell, ...
            [pozos(i,:), task.safe_tcp_z], sprintf('Sobre pozo %d', i), 0);
        [P, names, dwell] = agregarPunto(P, names, dwell, ...
            [pozos(i,:), task.plate_tcp_z], sprintf('Dispensacion pozo %d', i), task.dwell_disp_s);
        [P, names, dwell] = agregarPunto(P, names, dwell, ...
            [pozos(i,:), task.safe_tcp_z], sprintf('Salida pozo %d', i), 0);
    end
